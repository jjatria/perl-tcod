...
This function defines a transparent background color for this console. All
cells with this background color are ignored when calling L<blit|/blit>.
...
Reset all the cells in a console. After calling this method, all cells will
have their foreground and background colors set to the console's current
defaults (see L<set_default_foreground|/set_default_foreground> and friends),
and their ASCII code set to 32 (a blank space).
...
L<BackgroundFlag|TCOD/BackgroundFlag> enum), the alignment of the text (with
C<$align>, which must be a value in the L<Alignment|TCOD/Alignment> enum).
...
See L<set_color_control|/set_color_control> for how to define the value of
these codes.
...
This function calls the L<rect|/rect> function using the supplied parameters,
then draws a rectangle with the console's default foreground color.

The value of C<$str> is printed on the top of the rectangle, using inverted
colors.
...
The background color for every cell inside the rectangle will be adjusted with
the default background color of the console (the value in C<$flag> will
determine what effect this wil have on the cell's color).

If C<$clear> is set to a true value, their ASCII code will be set to 32 (space).

=head2 hline

    $console->hline( $x, $y, $length, $flag );

Draws a horizontal line in the console, using ASCII code 196
(C<TCOD::CHAR_HLINE>), and the console's default background/foreground colors.

=head2 vline

    $console->vline( $x, $y, $length, $flag );

Draws an vertical line in the console, using ASCII code 179
(C<TCOD::CHAR_VLINE>), and the console's default background/foreground colors.
...
L<FontFlags|TCOD/FontFlags> enum. The last two parameters determine the number
