...
L</get_origin> and L</get_destination> (see below).
...
See L</walk> for a way to move the internal iterator forward.
...
See L</compute> for how this is done.
...
destination. See L</compute> for how this is done.
...
L</size>, this can be used to iterate through a path without using the
internal iterator.
...
will be meaningful as long as L</is_empty> returns false.
